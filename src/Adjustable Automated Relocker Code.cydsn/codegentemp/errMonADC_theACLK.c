...
/*******************************************************************************
* Function Name: errMonADC_theACLK_Start
********************************************************************************
*
* Summary:
...
void errMonADC_theACLK_Start(void) 
{
    ...
    errMonADC_theACLK_CLKEN |= errMonADC_theACLK_CLKEN_MASK;
	errMonADC_theACLK_CLKSTBY |= errMonADC_theACLK_CLKSTBY_MASK;
}


/*******************************************************************************
* Function Name: errMonADC_theACLK_Stop
********************************************************************************
*
* Summary:
...
void errMonADC_theACLK_Stop(void) 
{
    ...
    errMonADC_theACLK_CLKEN &= (uint8)(~errMonADC_theACLK_CLKEN_MASK);
	errMonADC_theACLK_CLKSTBY &= (uint8)(~errMonADC_theACLK_CLKSTBY_MASK);
}
...
/*******************************************************************************
* Function Name: errMonADC_theACLK_StopBlock
********************************************************************************
*
* Summary:
...
{
    if ((errMonADC_theACLK_CLKEN & errMonADC_theACLK_CLKEN_MASK) != 0u)
    {
        ...
        CLK_DIST_AMASK = errMonADC_theACLK_CLKEN_MASK;
        CLK_DIST_DMASK = 0x00u;
#else
        CLK_DIST_DMASK = errMonADC_theACLK_CLKEN_MASK;
        CLK_DIST_AMASK = 0x00u;
#endif /* errMonADC_theACLK__CFG3 */

        /* Clear mask of bus clock. */
        CLK_DIST_BCFG2 &= (uint8)(~BCFG2_MASK);

        oldDivider = CY_GET_REG16(errMonADC_theACLK_DIV_PTR);
        ...
        CLK_DIST_LD = 0x00u;
        CY_SET_REG16(errMonADC_theACLK_DIV_PTR, oldDivider);
#endif /* HAS_CLKDIST_LD_DISABLE */
    }
}
...
/*******************************************************************************
* Function Name: errMonADC_theACLK_StandbyPower
********************************************************************************
*
* Summary:
...
/*******************************************************************************
* Function Name: errMonADC_theACLK_SetDividerRegister
********************************************************************************
*
* Summary:
                                ...
                                
{
    ...
    uint8 currSrc = errMonADC_theACLK_GetSourceRegister();
    uint16 oldDivider = errMonADC_theACLK_GetDividerRegister();

    if (clkDivider != oldDivider)
    {
        enabled = errMonADC_theACLK_CLKEN & errMonADC_theACLK_CLKEN_MASK;
        ...
        if ((currSrc == (uint8)CYCLK_SRC_SEL_CLK_SYNC_D) && ((oldDivider == 0u) || (clkDivider == 0u)))
        {
            ...
            if (oldDivider == 0u)
            {
                ...
                CY_SET_REG16(errMonADC_theACLK_DIV_PTR, clkDivider);
                errMonADC_theACLK_MOD_SRC &= (uint8)(~CYCLK_SSS);
            }
            else
            {
                /* Moving to SSS, set SSS which then ignores the divider and we can set     */
                /* it without bothering with the shadow load.                               */
                errMonADC_theACLK_MOD_SRC |= CYCLK_SSS;
                CY_SET_REG16(errMonADC_theACLK_DIV_PTR, clkDivider);
            }
        }
        else
        {
            ...
            if (enabled != 0u)
            {
                ...
                if (((errMonADC_theACLK_MOD_SRC & CYCLK_SYNC) == 0u) || (clkDivider == 0u))
                {
#if HAS_CLKDIST_LD_DISABLE
                    CY_SET_REG16(CYREG_CLKDIST_WRK0, oldDivider);
                    CLK_DIST_LD = CYCLK_LD_DISABLE|CYCLK_LD_SYNC_EN|CYCLK_LD_LOAD;
            ...
            else
            {
                ...
                CY_SET_REG16(errMonADC_theACLK_DIV_PTR, clkDivider);
				errMonADC_theACLK_CLKEN |= enabled;
            }
        }
    }
}
...
/*******************************************************************************
* Function Name: errMonADC_theACLK_GetDividerRegister
********************************************************************************
*
* Summary:
...
{
    return CY_GET_REG16(errMonADC_theACLK_DIV_PTR);
}


/*******************************************************************************
* Function Name: errMonADC_theACLK_SetModeRegister
********************************************************************************
*
* Summary:
...
{
    errMonADC_theACLK_MOD_SRC |= modeBitMask & (uint8)errMonADC_theACLK_MODE_MASK;
}


/*******************************************************************************
* Function Name: errMonADC_theACLK_ClearModeRegister
********************************************************************************
*
* Summary:
...
{
    errMonADC_theACLK_MOD_SRC &= (uint8)(~modeBitMask) | (uint8)(~(uint8)(errMonADC_theACLK_MODE_MASK));
}


/*******************************************************************************
* Function Name: errMonADC_theACLK_GetModeRegister
********************************************************************************
*
* Summary:
...
{
    return errMonADC_theACLK_MOD_SRC & (uint8)(errMonADC_theACLK_MODE_MASK);
}


/*******************************************************************************
* Function Name: errMonADC_theACLK_SetSourceRegister
********************************************************************************
*
* Summary:
...
{
    uint16 currDiv = errMonADC_theACLK_GetDividerRegister();
    uint8 oldSrc = errMonADC_theACLK_GetSourceRegister();
        ...
        (clkSource == ((uint8)CYCLK_SRC_SEL_CLK_SYNC_D))) && (currDiv == 0u))
    {
        ...
        errMonADC_theACLK_MOD_SRC |= CYCLK_SSS;
        errMonADC_theACLK_MOD_SRC =
            (errMonADC_theACLK_MOD_SRC & (uint8)(~errMonADC_theACLK_SRC_SEL_MSK)) | clkSource;
    }
...
/*******************************************************************************
* Function Name: errMonADC_theACLK_GetSourceRegister
********************************************************************************
*
* Summary:
...
{
    return errMonADC_theACLK_MOD_SRC & errMonADC_theACLK_SRC_SEL_MSK;
}
...
/*******************************************************************************
* Function Name: errMonADC_theACLK_SetPhaseRegister
********************************************************************************
*
* Summary:
...
{
    errMonADC_theACLK_PHASE = clkPhase & errMonADC_theACLK_PHASE_MASK;
}


/*******************************************************************************
* Function Name: errMonADC_theACLK_GetPhase
********************************************************************************
*
* Summary:
...
{
    return errMonADC_theACLK_PHASE & errMonADC_theACLK_PHASE_MASK;
}
