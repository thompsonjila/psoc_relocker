...
/*******************************************************************************
* Function Name: LFOutADC_theACLK_Start
********************************************************************************
*
* Summary:
...
void LFOutADC_theACLK_Start(void) 
{
    ...
    LFOutADC_theACLK_CLKEN |= LFOutADC_theACLK_CLKEN_MASK;
	LFOutADC_theACLK_CLKSTBY |= LFOutADC_theACLK_CLKSTBY_MASK;
}


/*******************************************************************************
* Function Name: LFOutADC_theACLK_Stop
********************************************************************************
*
* Summary:
...
void LFOutADC_theACLK_Stop(void) 
{
    ...
    LFOutADC_theACLK_CLKEN &= (uint8)(~LFOutADC_theACLK_CLKEN_MASK);
	LFOutADC_theACLK_CLKSTBY &= (uint8)(~LFOutADC_theACLK_CLKSTBY_MASK);
}
...
/*******************************************************************************
* Function Name: LFOutADC_theACLK_StopBlock
********************************************************************************
*
* Summary:
...
{
    if ((LFOutADC_theACLK_CLKEN & LFOutADC_theACLK_CLKEN_MASK) != 0u)
    {
        ...
        CLK_DIST_AMASK = LFOutADC_theACLK_CLKEN_MASK;
        CLK_DIST_DMASK = 0x00u;
#else
        CLK_DIST_DMASK = LFOutADC_theACLK_CLKEN_MASK;
        CLK_DIST_AMASK = 0x00u;
#endif /* LFOutADC_theACLK__CFG3 */

        /* Clear mask of bus clock. */
        CLK_DIST_BCFG2 &= (uint8)(~BCFG2_MASK);

        oldDivider = CY_GET_REG16(LFOutADC_theACLK_DIV_PTR);
        ...
        CLK_DIST_LD = 0x00u;
        CY_SET_REG16(LFOutADC_theACLK_DIV_PTR, oldDivider);
#endif /* HAS_CLKDIST_LD_DISABLE */
    }
}
...
/*******************************************************************************
* Function Name: LFOutADC_theACLK_StandbyPower
********************************************************************************
*
* Summary:
...
/*******************************************************************************
* Function Name: LFOutADC_theACLK_SetDividerRegister
********************************************************************************
*
* Summary:
                                ...
                                
{
    ...
    uint8 currSrc = LFOutADC_theACLK_GetSourceRegister();
    uint16 oldDivider = LFOutADC_theACLK_GetDividerRegister();

    if (clkDivider != oldDivider)
    {
        enabled = LFOutADC_theACLK_CLKEN & LFOutADC_theACLK_CLKEN_MASK;
        ...
        if ((currSrc == (uint8)CYCLK_SRC_SEL_CLK_SYNC_D) && ((oldDivider == 0u) || (clkDivider == 0u)))
        {
            ...
            if (oldDivider == 0u)
            {
                ...
                CY_SET_REG16(LFOutADC_theACLK_DIV_PTR, clkDivider);
                LFOutADC_theACLK_MOD_SRC &= (uint8)(~CYCLK_SSS);
            }
            else
            {
                /* Moving to SSS, set SSS which then ignores the divider and we can set     */
                /* it without bothering with the shadow load.                               */
                LFOutADC_theACLK_MOD_SRC |= CYCLK_SSS;
                CY_SET_REG16(LFOutADC_theACLK_DIV_PTR, clkDivider);
            }
        }
        else
        {
            ...
            if (enabled != 0u)
            {
                ...
                if (((LFOutADC_theACLK_MOD_SRC & CYCLK_SYNC) == 0u) || (clkDivider == 0u))
                {
#if HAS_CLKDIST_LD_DISABLE
                    CY_SET_REG16(CYREG_CLKDIST_WRK0, oldDivider);
                    CLK_DIST_LD = CYCLK_LD_DISABLE|CYCLK_LD_SYNC_EN|CYCLK_LD_LOAD;
            ...
            else
            {
                ...
                CY_SET_REG16(LFOutADC_theACLK_DIV_PTR, clkDivider);
				LFOutADC_theACLK_CLKEN |= enabled;
            }
        }
    }
}
...
/*******************************************************************************
* Function Name: LFOutADC_theACLK_GetDividerRegister
********************************************************************************
*
* Summary:
...
{
    return CY_GET_REG16(LFOutADC_theACLK_DIV_PTR);
}


/*******************************************************************************
* Function Name: LFOutADC_theACLK_SetModeRegister
********************************************************************************
*
* Summary:
...
{
    LFOutADC_theACLK_MOD_SRC |= modeBitMask & (uint8)LFOutADC_theACLK_MODE_MASK;
}


/*******************************************************************************
* Function Name: LFOutADC_theACLK_ClearModeRegister
********************************************************************************
*
* Summary:
...
{
    LFOutADC_theACLK_MOD_SRC &= (uint8)(~modeBitMask) | (uint8)(~(uint8)(LFOutADC_theACLK_MODE_MASK));
}


/*******************************************************************************
* Function Name: LFOutADC_theACLK_GetModeRegister
********************************************************************************
*
* Summary:
...
{
    return LFOutADC_theACLK_MOD_SRC & (uint8)(LFOutADC_theACLK_MODE_MASK);
}


/*******************************************************************************
* Function Name: LFOutADC_theACLK_SetSourceRegister
********************************************************************************
*
* Summary:
...
{
    uint16 currDiv = LFOutADC_theACLK_GetDividerRegister();
    uint8 oldSrc = LFOutADC_theACLK_GetSourceRegister();
        ...
        (clkSource == ((uint8)CYCLK_SRC_SEL_CLK_SYNC_D))) && (currDiv == 0u))
    {
        ...
        LFOutADC_theACLK_MOD_SRC |= CYCLK_SSS;
        LFOutADC_theACLK_MOD_SRC =
            (LFOutADC_theACLK_MOD_SRC & (uint8)(~LFOutADC_theACLK_SRC_SEL_MSK)) | clkSource;
    }
...
/*******************************************************************************
* Function Name: LFOutADC_theACLK_GetSourceRegister
********************************************************************************
*
* Summary:
...
{
    return LFOutADC_theACLK_MOD_SRC & LFOutADC_theACLK_SRC_SEL_MSK;
}
...
/*******************************************************************************
* Function Name: LFOutADC_theACLK_SetPhaseRegister
********************************************************************************
*
* Summary:
...
{
    LFOutADC_theACLK_PHASE = clkPhase & LFOutADC_theACLK_PHASE_MASK;
}


/*******************************************************************************
* Function Name: LFOutADC_theACLK_GetPhase
********************************************************************************
*
* Summary:
...
{
    return LFOutADC_theACLK_PHASE & LFOutADC_theACLK_PHASE_MASK;
}
